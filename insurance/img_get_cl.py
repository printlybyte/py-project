...
class Txt():
    ...
    def get_contents_list_img(self, target):
        ...
        try:
            ...
            ab_f = BeautifulSoup(trhtml, "html.parser")
            ab = ab_f.find_all('input')
            for va in ab:
                im = va.get('data-src')
                print(im)
                self.urlsimg.append(im)
            # texts = texts[0].text.replace('\xa0' * 8, '\n\n')
            return texts

    def download_img(self, img_url, imglens):
        print(img_url)
        ...
        if r.status_code == 200:
            open('E:\\image' + '\\' + filename, 'wb').write(r.content)  # 将内容写入图片
            print(len(self.urlsimg), "正在下载...第", imglens, '张')
        del r
    ...
    def get_chapter_list(self, baseUrl, mUrl):
        ...
        for key in htre[9:]:
            ...
            self.get_contents_list_img(baseUrl + ab_urs[0])
...
if __name__ == '__main__':
    ...
    mT.get_chapter_list('http://private70.ghuws.win/',
                        'http://private70.ghuws.win/thread0806.php?fid=16&search=&page=1')
    # mT.get_contents_list_img('http://private70.ghuws.win/htm_mob/16/1904/3492867.html')
    # print('《x》开始下载：')
    # lens=sum(mT.nums)
    # print(lens)
    # for i in range(lens):
    ...
    #     sys.stdout.flush()
    # print('《x》下载完成')
    imglen=len(mT.urlsimg)
    for img in range(0,imglen):
        mT.download_img(mT.urlsimg[img],img)
    print(len(mT.urlsimg))
